...
Eight Vision-Language Models were evaluated: six contrastive (CLIP-family) and two generative (BLIP-2, LLaVA).
...
**Generative models (BLIP-2, LLaVA):**
...
| BLIP-2 (OPT-2.7B) | 67.10 | 75.29 | 53.35 | **44.81**† | 59.70 |
| LLaVA-1.5 (7B) | 92.15 | **100.00** | 28.54 | 22.67 | 37.40 |

> † BLIP-2 achieved 41.36% on IN-A; **CLIP ViT-L/14** achieved the highest IN-A at 44.81%.
...
| BLIP-2 (OPT-2.7B) | — | — | SKIP | — |
| LLaVA-1.5 (7B) | — | — | SKIP | — |

> Generative models (BLIP-2, LLaVA) skipped corruption datasets due to per-image generation latency.
...
6. **LLaVA-1.5 achieves 100% on ScrewSet** — the generative approach with class names provided in the prompt is dramatically more effective for domain-specific classification with few classes (40 screw types). However, this comes with a significant caveat: the fuzzy matching with only 40 classes makes matching much easier than with 1000 ImageNet classes.

7. **BLIP-2 achieves 75.29% on ScrewSet**, also far exceeding all contrastive models, confirming the advantage of generative VQA-style evaluation when class names are descriptive and few.
...
| Phase 3 (VLM, zero-shot) | 98.48% (EVA-02) | 71.36% (EVA-02) | 0.4610 (EVA-02) | 100% (LLaVA)* |

> *LLaVA's 100% ScrewSet accuracy uses generative VQA with class names in the prompt (40 classes); Phase 1/2 ScrewSet results used supervised fine-tuning on training data.
...
- For **domain-specific tasks** (ScrewSet), generative VLMs with class prompting dramatically outperform contrastive VLMs, approaching fine-tuned model performance
...
**48 JSON files** (8 models × 6 datasets) — all runs completed successfully.
